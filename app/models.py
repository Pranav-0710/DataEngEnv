...
class Action(BaseModel):
    action_type: str = Field(description="One of: inspect_data | run_script | edit_script | check_schema | submit")
    payload: dict = Field(description="Action payload details like patch content etc.")

    @field_validator("action_type")
    @classmethod
    def validate_action_type(cls, v: str) -> str:
        allowed = {"inspect_data", "run_script", "edit_script", "check_schema", "submit"}
        if v not in allowed:
